...
def resolve_torch_device(device: str = "cuda") -> str:
...
def build_brand_embeddings_from_ads(
    all_metadata: pd.DataFrame,
    all_embeddings: Dict[str, np.ndarray],
) -> Dict[str, np.ndarray]:
    if all_metadata is None or all_metadata.empty or not all_embeddings:
        return {}

    brand_vectors: Dict[str, List[np.ndarray]] = {}
    for _, row in all_metadata.iterrows():
        brand_id = str(row.get("brand_id", ""))
        ad_id = str(row.get("ad_id", ""))
        emb = all_embeddings.get(ad_id)
        if not brand_id or emb is None:
            continue
        brand_vectors.setdefault(brand_id, []).append(np.asarray(emb, dtype=float))

    return {
        brand_id: np.mean(vectors, axis=0)
        for brand_id, vectors in brand_vectors.items()
        if vectors
    }
...
def find_most_similar_brand(target_brand_id: str, all_metadata: pd.DataFrame, metric_modes: List[str], all_embeddings: Dict[str, np.ndarray] = None,    brand_embeddings: Dict[str, np.ndarray] = None) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    if not brand_embeddings and all_embeddings:
        print("[Cross-Brand] Building brand embeddings from ad embeddings")
        brand_embeddings = build_brand_embeddings_from_ads(all_metadata, all_embeddings)

    use_brand_emb = brand_embeddings is not None and len(brand_embeddings) > 0
    
    target_brand_id_str = str(target_brand_id)

    if not use_brand_emb or target_brand_id_str not in brand_embeddings:
        print(f"[Cross-Brand] No embedding available for target brand {target_brand_id}")
        return None, []
...
def train_lgbm_ranker(train_df: pd.DataFrame, target_col: str, direction: str = "higher", llm_feature_cols: List[str] = None, tune_hyperparams: bool = False):
    if lgb is None:
        raise ImportError("lightgbm is required for model_type='lgbm'. Install it with `pip install lightgbm`.")
...
def run_brand_pipeline(brand_id: str, train_df: pd.DataFrame,  test_df: pd.DataFrame,
    ...
    for mode in metric_modes:
        ...
        for model_type in all_model_types:
            if model_type == 'lgbm':
                if lgb is None:
                    print(f"[{brand_id}][{mode}] WARNING: lightgbm is not installed. Skipping LGBM.")
                    continue
